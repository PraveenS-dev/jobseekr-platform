...
import { FaArrowLeft, FaMapMarkerAlt, FaMoneyBillAlt, FaFilePdf, FaUpload } from 'react-icons/fa';
        ...
        <div className="max-w-full mx-auto px-4 py-10">
            ...
            {jobs && (
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-6 mb-8">
                    <h3 className="text-2xl font-bold text-gray-800 dark:text-white mb-4">{jobs.title}</h3>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 text-gray-700 dark:text-gray-300 text-sm">
                        <p className="flex items-center gap-2"><FaMapMarkerAlt className="text-blue-500" /> {jobs.location}</p>
                        <p className="flex items-center gap-2"><FaMoneyBillAlt className="text-green-500" /> ₹{jobs.salary}</p>
                        ...
                        <p><strong>Posted At:</strong> {dayjs(jobs.created_at).format("DD MMM YYYY, hh:mm A")}</p>
                    </div>

                    <div className="mt-4">
                        <p className="text-gray-700 dark:text-gray-300"><strong>Description:</strong></p>
                        <p className="text-gray-600 dark:text-gray-400 text-sm">{jobs.description}</p>
                    </div>
                </div>
            )}
                    ...
                    {previewUrl && (
                        <div className="mt-2 w-full">
                            <h5 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-2">
                                <FaFilePdf className="text-red-500" /> Resume Preview
                            </h5>
